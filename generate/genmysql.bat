...
if not "%1"=="MySQL-5.6" "%OSP_ROOT_DIR%modules\%1\bin\mysqld.exe" --defaults-file="%OSP_ROOT_DIR%modules\%1\my.ini" --initialize-insecure --console --standalone
timeout /t 3 /nobreak > nul
del "%OSP_ROOT_DIR%modules\%1\*.ini" /q 2>nul
copy "%OSP_ROOT_DIR%generate\setup\private_key.pem" "%OSP_ROOT_DIR%data\%1\default\private_key.pem"
copy "%OSP_ROOT_DIR%generate\setup\public_key.pem" "%OSP_ROOT_DIR%data\%1\default\public_key.pem"
call osp on %1
...
if not "%1"=="MySQL-5.6" if not "%1"=="MySQL-5.7" mysql --defaults-file="%OSP_ROOT_DIR%modules\%1\my.ini" --protocol=PIPE --socket=%1 -u root mysql < "%OSP_ROOT_DIR%generate\setup\timezone_posix.sql"
...
call osp use %1
timeout /t 5 /nobreak > nul
mysql --defaults-file="%OSP_ROOT_DIR%modules\%1\my.ini" --force --protocol=PIPE --socket=%1 --host="" -u root mysql < "%OSP_ROOT_DIR%generate\setup\install.sql"
call osp off %1
timeout /t 3 /nobreak > nul
